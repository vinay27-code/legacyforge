...
@Component({
          ...
          <section class="viewer">
            @if (currentFile()) {
              ...
              @if (currentFile()!.binary || currentFile()!.content === null) {
                ...
                <pre class="code"><code>{{ currentFile()!.content }}</code></pre>
              }
            } @else {
  ...
  styles: [`
    ...
    .page { display: flex; flex-direction: column; height: calc(100vh - 128px); }
...
export class RepoDetailComponent implements OnInit {
  ...
  openFile(path: string): void {
    ...
    const id = this.route.snapshot.paramMap.get('id')!;
    this.service.file(id, path).subscribe((content) => this.currentFile.set(content));
  }
